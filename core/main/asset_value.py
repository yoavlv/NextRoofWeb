...
from sklearn.ensemble import GradientBoostingRegressor, RandomForestRegressor, StackingRegressor  # noqa: F401
...
from .base_functions import lasted_deals_street  # noqa: E402


def asset_value_page(request):

    df_history_deals = pd.read_csv('core/data/Nadlan_clean.csv')
...
def calc_asset_value(request):
    ...
    if request.method == 'GET':
        ...
        df = pd.read_csv('core/data/Nadlan_clean.csv')
        neighborhood_plt = Price_Increases_Neighborhood(
            df, params['Neighborhood'])
...
def check_for_match(df, params, number, target, col_1, col_2=None):
    number = int(number)
    number_offsets = [
        number, number - 1, number + 1, number + 2, number - 2, number + 3,
        number - 3, number + 4, number - 4, number + 5, number - 5
    ]
    for number in number_offsets:
        if col_2:
            try:
                match = df.loc[(df[col_1] == params[col_1]) &
                               (df[col_2] == number), target].values[0]
                return match
            except:
                pass
        else:
...
def calc_missing_values(df, params):
    ...
    params['Lat'] = check_for_match(df, params, params['Home_number'], 'y',
                                    'Street', 'Home_number')
    params['Long'] = check_for_match(df, params, params['Home_number'], 'x',
                                     'Street', 'Home_number')

    params['Gush'] = check_for_match(df, params, params['Home_number'], 'Gush',
                                     'Street', 'Home_number')
    params['Helka'] = check_for_match(df, params, params['Home_number'],
                                      'Helka', 'Street', 'Home_number')

    params['Distance_sea'] = calc_distance_from_the_see_TLV(
        params['Long'], params['Lat'])
    params['Neighborhood'] = add_neighborhood(params['Street'])
    params['Train'] = calc_distance_from_train_station(params['Long'],
                                                       params['Lat'])

    nadlan_df = pd.read_csv("core/data/Nadlan_clean.csv", index_col=0)
    params['Build_year'] = check_for_match(nadlan_df, params, params['Helka'],
                                           'Build_year', 'Gush', 'Helka')

    params['Floors'] = check_for_match(nadlan_df, params, params['Helka'],
                                       'Floors', 'Gush', 'Helka')

    params['Age'] = 2023 - params['Build_year']

    params['Street_rank'] = nadlan_df.loc[(
        nadlan_df['Street'] == params['Street']), 'Street_rank'].max()
    params['Gush_rank'] = nadlan_df.loc[(nadlan_df['Gush'] == params['Gush']),
                                        'Gush_rank'].max()

    params['Helka_rank'] = check_for_match(nadlan_df, params, params['Helka'],
                                           'Helka_rank', 'Gush', 'Helka')

    params['Neighborhood_rank'] = nadlan_df.loc[(
        nadlan_df['Neighborhood'] == params['Neighborhood']),
                                                'Neighborhood_rank'].max()
...
def predict_apt_price(params, model):
    ...
    X = X.reindex(columns=[
        "Rooms", "Floor", "Size", "Build_year", "Floors", "Long", "Lat",
